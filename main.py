...
# DEPENDENCIES - see notes (all examples using the same params to be injected)
## Can be added as route handler using Depends(dependency_name, use_cache=True/False)
# #     OR
# in path operation decorator as a list --> dependencies = [Depends(), Depends() etc]
# and even as Global Dependency app = FastAPI(dependencies = [Depends(), Depends() etc])
## Functions as Dependencies (eg. returns a dict)
...
async def get_items_depends(arg: dict = Depends(common_params)):
    return arg


@app.get("/users_depends")
async def get_users_depends(arg: CommonQueryParams = Depends(CommonQueryParams)):
    # calls and creates instance of class which gets passed as param (note CommonQueryParams present twice. first time is for pydantic to validate the data and second time is to pass the data to the route function as a param.)
    response = {}
    if arg.q:
        response.update({"q": arg.q})
    items = fake_users_db[arg.skip : arg.skip + arg.limit]
    response.update({"items": items})
    return response


# ADDING DEPENDENCIES IN DECORATOR
## Note even if they have return values they won't be passed/used in the actual path function
async def verify_token(x_token: str = Header(...)):
    if x_token != "fake-super-secret-token":
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST, detail="X-Token header invalid"
        )


async def verify_key(x_key: str = Header(...)):
    if x_key != "fake-super-secret-key":
        raise HTTPException(status.HTTP_400_BAD_REQUEST, detail="X-Key header invalid")
    return x_key


@app.get(
    "/decorator_depends", dependencies=[Depends(verify_token), Depends(verify_key)]
)
async def read_decorator_depends():
    return {"message": "Decorator dependency token and key valid"}


#  In both cases (function, class dependency) FastAPI uses the params to 'solve' the depenedency <option 'q' query, skip/limit int w/default params> *Note* when used in decorator will be 'solved' the same way but value won't be passed to path function
# In all cases, data will be converted, validated, documentated in autogenerated OpenAPI schema
# DEPENDENCIES WITH YIELD
# FastAPI supports dependencies that do something after solving dependency *(use 'yield' instead of return) eg. create a db session + close it after finishing. The yielded value gets injected into path operation + other dependencies
# async def get_db():
# db = DBSession()      try:yield db    finally: db.close
